...
const Schema_Amount = require("../model/secondroommodel");

app.post("/second_room", async (req, res) => {
...
app.get("/second_room_getamountdata", async (req, res) => {
  await Schema_Amount.find()
    .then((result) => {
...
app.post("/second_room_updateamountdata/:id", async (req, res) => {
  ...
  await Schema_Amount.find({
    ...
    .then(async (result) => {
      ...
      if (result.length === 1) {
        return res.json({
          isValid: false,
          message: "this room is allready book.....",
        });
      } else {
        await Schema_Amount.findByIdAndUpdate(
          ...
          .catch((error) => {
            return res.json({
              isValid: false,
              message: "data in no update....." + error,
            });
          });
      }
    })
...
app.delete("/second_room_deleteamountdata/:id", async (req, res) => {
